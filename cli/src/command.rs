		...
		Some(Subcommand::TryRuntime(cmd)) => {
			...
			#[cfg(feature = "cherry-native")]
			{
				return runner.async_run(|_| {
					Ok((
						cmd.run::<service::cherry_runtime::Block, HostFunctionsOf<service::CherryExecutorDispatch>, _>(
							Some(timestamp_with_babe_info(service::cherry_runtime_constants::time::MILLISECS_PER_BLOCK))
						)
						.map_err(Error::SubstrateCli),
						task_manager,
					))
